...
ENV ALT_IMG_DATE=20190312
ENV ALT_IMG_ARC=i586

WORKDIR /tmp/
RUN \
    wget --no-check-certificate --progress=bar:force --output-document=basealt.tar.xz https://mirror.yandex.ru/altlinux/p8/images/starterkits/alt-p8-ovz-generic-${ALT_IMG_DATE}-${ALT_IMG_ARC}.tar.xz && \
    mkdir -p /alt/ && \
